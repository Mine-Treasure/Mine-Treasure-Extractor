...
export default class VariableBlockMapExtractor extends BaseExtractor {
    ...
    private readonly INIT_FILE = this.getRelativePath('data/mt/functions/init.mcfunction');
    ...
    public async Extract(): Promise<unknown> {
        ...
        const blockVariableRegex = /scoreboard objectives add mt\.(.+) minecraft\.mined:(.+)/g;
        const stoneCheckFile_contents = await readFile(this.INIT_FILE, 'utf-8');


        let m;
        while ((m = blockVariableRegex.exec(stoneCheckFile_contents)) !== null) {
            ...
            if (!this.passingData["mineable_blocks"].includes(block)) this.passingData["mineable_blocks"].push(block);
        }
